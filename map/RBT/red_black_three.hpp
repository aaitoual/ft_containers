...
namespace ft
{
	...
	public:
		...
		NODE *get_next()
		{
			...
			else if (parent != NULL && parent->content > content)
			{
				return parent;
			}
			else if (parent != NULL && parent->content < content)
	...
	public:
		...
		{
			__ROOT_P = __alloc_obj.allocate(1);
			__alloc_obj.construct(__ROOT_P, T());
		}
		NODE<T> *get_first()
		{
			NODE<T> *ret = __root;
		...
		void fix_the_three(NODE<T> *node)
		{
			...
			if (parent != __root)
				...
				return;
			while (node != __root && node != __nullnode && parent != __nullnode && parent->color == 1)
			{ // if the parent is red and the node isn't root yet
				if (which_node(parent) == 2)
				{ // if the parent is the left of the G_parent
					if (G_parent != __nullnode && G_parent->right != __nullnode && G_parent->right->color == 1)
					{ // if the uncle is red
						G_parent->color = 1;
						parent->color = 0;
